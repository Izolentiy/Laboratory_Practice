...
void RCC_Ini() {
    ...
    // устанавливаем множитель N = 200
    // сбрасываем выходной предделитель P = 4 [0b01 << 16]
    CLEAR_REG(RCC->PLLCFGR);
    SET_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLSRC_HSE);
    MODIFY_REG(RCC->PLLCFGR, RCC_PLLCFGR_PLLM, 4);
    MODIFY_REG(RCC->PLLCFGR, RCC_PLLCFGR_PLLN, 200 << 6);
...
void SysTick_Ini() {
    ...
    MODIFY_REG(SysTick->LOAD, SysTick_LOAD_RELOAD_Msk, 99999 << SysTick_LOAD_RELOAD_Pos);
    MODIFY_REG(SysTick->VAL, SysTick_VAL_CURRENT_Msk, 99999 << SysTick_VAL_CURRENT_Pos);
